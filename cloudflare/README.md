...
- 管理キーを持つ人は、このサービス内の全共有の作成・一覧・保存内容の確認・削除が可能。一般公開の投稿サービスではなく、管理キーを配った記録者向け。共用PCでは設定画面でキーを解除する。
- 有効期限は7/30/90/365日または無期限（初期値30日）。期限切れは即時閲覧不可、期限切れデータは日次処理で削除する。設定画面の管理画面では共有内容を確認でき、削除時は共有行（本文・パスワード検証値を含む）を即時削除する。すでに相手が保存した画像やコピー、Cloudflareのバックアップ保持分は別途消去できない。
