...
try {
  ...
  await page.evaluate(() => {
    ...
    Object.defineProperty(navigator, 'share', { configurable: true, value: async data => { const file = data.files[0]; window.sharedFile = { name: file.name, size: file.size, type: file.type, text: file.type === 'application/json' ? await file.text() : null, message: data.text || '' }; } });
  });
  await page.getByRole('button', { name: '共有', exact: true }).click();
  await page.getByRole('button', { name: '閲覧用レポートを共有', exact: true }).click();
