...
const output = `${process.env.BROWSER_OUTPUT_DIR || 'test-results'}/${engine}-${Date.now()}`;
...
try {
  ...
  await page.locator('#substitution-form [name=outPlayerId]').nth(0).check();
  await page.locator('#substitution-form [name=outPlayerId]').nth(1).check();
  await page.locator('#substitution-form [name=inPlayerId]').nth(0).check();
  await page.locator('#substitution-form [name=inPlayerId]').nth(1).check();
  await page.getByRole('button', { name: '交代を記録', exact: true }).click(); await ready();
  let data = await snapshot();
  const substitutions = data.events.filter(event => event.eventType === 'SUB' && !event.deletedAt);
  assert.equal(substitutions.length, 2);
  assert.equal(new Set(substitutions.map(event => event.substitutionBatchId)).size, 1);
  await page.getByRole('button', { name: /UNDO/ }).click(); await ready();
  data = await snapshot(); assert.equal(data.events.filter(event => event.eventType === 'SUB' && !event.deletedAt).length, 0);
  step('all 13 stats, automatic score, opponent scoring and multi-player substitution UNDO work');
  ...
  await route('#settings'); await page.locator('#advancedMode').check(); await ready();
  await route(liveHash); await page.getByRole('button', { name: '2ポイント失敗', exact: true }).click();
  await page.locator('#sheet [data-action=pick-player]').filter({ hasText: '#4山田' }).click();
  await page.getByRole('heading', { name: '2P × · 位置', exact: true }).waitFor();
  assert.equal(await page.locator('#sheet .shot-map-zone').count(), 12);
  assert.equal(await page.locator('#sheet .shot-map-zone.active').count(), 7);
  assert.equal(await page.locator('#sheet .shot-court-map text').count(), 0);
  await page.screenshot({ path: `${output}/shot-map.png` });
  await page.locator('#sheet [data-action=shot-zone][data-zone=two-top]').click(); await ready();
  assert.equal((await snapshot()).events.sort((a, b) => a.seq - b.seq).at(-1).shotZone, 'two-top');
  await route('#settings'); await page.locator('#advancedMode').uncheck(); await page.locator('#continuous').check(); await ready();
  step('Advanced mode uses the 12-zone B.League-style court map without visible labels');
  ...
  assert.equal(await page.locator('.aggregate-game-scores .aggregate-game-score').count(), 2); assert.equal(await page.locator('.total-row .pts-cell').textContent(), '13');
  ...
  await page.locator('#sheet [data-action=select-aggregate-player-game]').selectOption('total');
  assert.ok((await page.locator('#sheet').textContent()).includes('全試合集計')); await dismiss();
