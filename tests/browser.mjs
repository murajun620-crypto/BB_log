...
try {
  ...
  step('team/game drafts survive reload; team, seven players and starting five created');
  await page.getByRole('button', { name: '試合メニュー', exact: true }).click();
  await page.getByRole('button', { name: 'メンバーを追加', exact: true }).click();
  await page.locator('#sheet [data-action=pick-member]').filter({ hasText: '#30中村' }).click(); await ready();
  assert.equal((await snapshot()).games.find(g => g.id === liveHash.split('/')[1]).roster.length, 7);
  step('registered player can be added after game start and appears in game roster');
