...
import { shotChartMapHTML, shotMarkerDetailFeedbackHTML, BOX_STAT_KEYS as BOX_STATS, DETAIL_STAT_KEYS as DETAIL_STATS, statLabel, statValue, sortPlayersByNumber } from '../../js/views.js';
...
function shooting(stats, games = 1) {
  return `<div class="shooting-grid">${[['FG', 'FGM', 'FGA'], ['2P', 'P2M', 'P2A'], ['3P', 'P3M', 'P3A'], ['FT', 'FTM', 'FTA']].map(([label, made, attempts]) => `<div><span>${label}</span><strong>${stats[made]}<small>/${stats[attempts]}</small></strong><b>${percent(stats[made], stats[attempts])}</b>${games > 1 ? `<em class="shooting-average">${average(stats[made], games)}/${average(stats[attempts], games)}</em>` : ''}</div>`).join('')}</div>`;
}
...
function statCells(stats, games = 1) {
  return BOX_STATS.map(key => `<span><small>${statLabel(key)}</small><b>${metric(statValue(stats, key), games)}</b></span>`).join('');
}
...
function readerReportView(data) {
  ...
  const status = view.status === 'live' ? '記録時点' : 'FINAL';
  const mode = modeText();
  const reportTitle = data.title || data.tournamentName || 'SHARED BOX SCORE';
  const reportNote = data.note ? `<p class="shared-report-note">${esc(data.note)}</p>` : '';
  const label = statLabel;
  const cell = (stats, key) => `<td${key === 'PTS' ? ' class="pts-cell"' : ''}>${metric(statValue(stats, key), gameCount)}</td>`;
  const rows = sortPlayersByNumber(view.players).map(player => `<tr><th scope="row"><button type="button" data-player-id="${esc(player.id)}" aria-label="${esc(player.number)} ${esc(player.name)}の詳細を開く"><b>${esc(player.number)}</b><span>${esc(player.name)}</span></button></th>${BOX_STATS.map(key => cell(player.stats, key)).join('')}</tr>`).join('');
  ...
  const gameScores = detailGames.length > 1 ? `<section class="report-card game-list-card"><div class="game-meta"><span>${esc(reportTitle || data.teamName)} · 各試合のスコア</span>${currentGame ? '<button class="mode-toggle" type="button" data-action="select-game" data-game-index="-1">全試合集計</button>' : '<span class="report-hint">タップで試合を表示</span>'}</div><div class="aggregate-game-scores">${detailGames.map((game, index) => `<button class="aggregate-game-score ${index === selectedGameIndex ? 'selected' : ''}" type="button" data-action="select-game" data-game-index="${index}" aria-label="${esc(formatDate(game.date))} ${esc(game.opponentName)}の試合を表示"><span class="aggregate-game-info"><b>${esc(formatDate(game.date))}</b><small>${esc(game.teamName)} vs. ${esc(game.opponentName)}</small></span><strong class="game-score">${game.team.PTS}<span>–</span>${game.opponentScore}</strong><span class="game-score-arrow">›</span></button>`).join('')}</div></section>` : '';
  const scoreBlock = detailGames.length > 1 ? `${gameScores}${currentGame ? scoreCard : ''}` : scoreCard;
  return `<main class="reader-shell"><header class="reader-header"><div class="identity"><span class="brand-mark">C</span><div><strong>COURTSIDE</strong><span>READER</span></div></div><span class="read-only">閲覧専用</span></header><section class="intro-line"><span>${esc(reportTitle)}</span><span>${esc(formatDate(view.date))} · ${esc(view.format)}</span></section>${reportNote}${scoreBlock}${shotChart(view.shots || [], null, shotDisplayMode, view.players)}<div class="section-heading"><h2>チーム・シューティング</h2>${gameCount > 1 ? '<span class="muted">成功率は合計から計算</span>' : ''}</div><div class="panel">${shooting(view.team, gameCount)}</div><div class="rebound-total"><span>OR <b>${metric(view.team.OREB, gameCount)}</b></span><span>DR <b>${metric(view.team.DREB, gameCount)}</b></span><span>REB <b>${metric(view.team.REB, gameCount)}</b></span></div><div class="section-heading"><h2>選手スタッツ</h2>${gameCount > 1 ? `<button class="mode-toggle" type="button" data-action="toggle-stat-mode" aria-label="合計と平均を切り替え">合計 / 平均：${mode}</button>` : '<span class="muted">選手をタップで詳細</span>'}</div><div class="box-table-wrap"><table class="box-table"><thead><tr><th scope="col">PLAYER</th>${BOX_STATS.map(key => `<th scope="col">${label(key)}</th>`).join('')}</tr></thead><tbody>${rows}<tr class="total-row"><th scope="row">TEAM</th>${BOX_STATS.map(key => cell(view.team, key)).join('')}</tr></tbody></table></div><footer class="reader-footer">閲覧専用。端末には保存されません。</footer></main>`;
}

function landingView() {
  return `<main class="landing"><header class="reader-header"><div class="identity"><span class="brand-mark">C</span><div><strong>COURTSIDE</strong><span>READER</span></div></div><span class="read-only">閲覧専用</span></header><section class="landing-card"><span class="landing-icon">↗</span><p>共有リンクを受け取りました</p><h1>BOX SCOREを<br>表示します。</h1><p class="landing-note">チーム登録、試合入力、端末内データの保存は行いません。リンクに含まれる試合結果だけを読み取ります。</p></section><p class="landing-help">共有リンクをSafariで開いた後、必要なら「ホーム画面に追加」でCourtside Readerとして使えます。</p></main>`;
}
...
function openPlayer(playerId) {
  ...
  playerDialog.innerHTML = `<div class="dialog-handle"></div><button class="dialog-close" type="button" data-close-dialog aria-label="閉じる">×</button>${options}<p class="dialog-context">${esc(context)}</p><div class="player-detail"><span class="jersey">${esc(player.number)}</span><div><h2 id="player-dialog-title">${esc(player.name)}</h2><p><b>${playerValue(player.stats.PTS)}</b> PTS</p></div></div>${shooting(player.stats, selectedCount)}<div class="detail-stats">${DETAIL_STATS.map(key => `<span><small>${statLabel(key)}</small><b>${playerValue(statValue(player.stats, key))}</b></span>`).join('')}</div>${shotChart(source.shots || [], playerId, shotDisplayMode, source.players)}`;
