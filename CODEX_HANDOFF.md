...
- GitHub Pagesのブランチ連動Run 66〜68とカスタムRun 1/2は旧デプロイとの競合で失敗したが、PagesをUnpublishしてActions公開へ切り替え、停滞デプロイの自動キャンセルを追加したカスタムRun 3（コミット `ad3655c`）は成功。公開URLはLive。既存ブラウザでは旧Service Workerが残る場合があるため、更新後は全画面を閉じて再起動し、設定画面のバージョン表示を確認する。
