...
- プッシュ後にGitHub Pagesデプロイを必ず作成し、完了・成功まで確認する運用を `AGENTS.md` に追加。機能コミット `33ac148` はPages Run 5、引き継ぎ書更新コミット `fe2e0fb` はRun 6、作戦ボード機能コミット `7113b9b` はRun 8で成功し、公開URLへの反映を確認した。
...
- GitHub Pagesの旧ブランチ連動RunやカスタムRun 1/2は旧デプロイとの競合で失敗したが、PagesをUnpublishしてActions公開へ切り替え、停滞デプロイの自動キャンセルを追加した。その後のRun 3（`ad3655c`）、Run 4（`73cf2d4`）、Run 5（`33ac148`）、Run 6（`fe2e0fb`）、Run 7（`0085988`）、Run 8（`7113b9b`）は成功。公開URLはLive。既存ブラウザでは旧Service Workerが残る場合があるため、更新後は全画面を閉じて再起動し、設定画面のバージョン表示を確認する。
