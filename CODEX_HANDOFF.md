...
バスケットボールの試合記録・共有を行うオフライン対応PWA。正規リポジトリは `Documents/Github/BB_log`、公開先は `https://murajun620-crypto.github.io/BB_log/`。アプリは2.0.7。AdvancedモードでBリーグ式の12ゾーンにシュート位置を記録し、結果画面・選手詳細・共有Readerにショットチャートを表示する。
...
- シュート位置入力をBリーグ図に近い横長SVGコートへ変更。図中の文字をなくし、ゴール下、ペイント内、2Pの左右コーナー／左右ウイング／トップ、3Pの同5方向を色と実際のコート形状で選ぶ。旧13ゾーンは新12ゾーンへ読み替えて互換表示する。2P左右コーナーを3Pラインの直線部分沿いの短い帯へ縮小し、左右ウイングを曲線部分まで広げた。2Pトップとの境界は外側へ約30°傾けている。入力画面では3P側も色分けし、結果画面とReaderは同じコート画像に成功数／試投数・成功率を表示する。バージョンは2.0.7。
...
- 単体テスト21件・構文チェック・Chromeブラウザ回帰テスト・Cloudflare共有ブラウザテストは通過。Cloudflare短縮リンクの公開後疎通はユーザー環境で確認する。
