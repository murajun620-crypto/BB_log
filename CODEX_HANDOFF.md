...
- 2.0.9でCloudflare共有管理を拡張。設定画面から保存スナップショットの概要・allowlist済みJSONを確認でき、共有停止時はD1の共有行を即時削除する。本番Workerへ反映し、架空データで確認200・削除200・削除後404を確認した。公開WorkerのVersion IDは `0e4a8e92-8cf8-4fae-a946-4284f5e67bbc`。
