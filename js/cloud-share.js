...
export const createCloudShare = (report, { password = '', days = 30, title, note } = {}) => request('/v1/shares', { method: 'POST', token: admin(), data: { report, password, days, ...(title === undefined ? {} : { title }), ...(note === undefined ? {} : { note }) } });
