import { STATS, STAT_DEFS, SHOT_ZONES, aggregate, aggregateGames, activeEvents, eventLabel, formatGame, isShotEvent, lineup, localDate, normalizeShotZone, percent } from './domain.js';
...
const PRO_COURT_MARKINGS = '<path d="M24 24H916V476H24ZM470 24V476M24 105H150M24 395H150M150 105C270 130 330 195 330 250C330 305 270 370 150 395M916 105H790M916 395H790M790 105C670 130 610 195 610 250C610 305 670 370 790 395M50 170H180V330H50M760 170H890V330H760M50 205H180M50 295H180M760 205H890M760 295H890M55 215V285M885 215V285M55 250H38M885 250H902"></path><circle cx="470" cy="250" r="42"></circle><circle cx="70" cy="250" r="11"></circle><circle cx="870" cy="250" r="11"></circle><circle cx="180" cy="250" r="60"></circle><circle cx="760" cy="250" r="60"></circle>';
export function clockText(seconds) {
  if (!Number.isFinite(seconds)) return '--:--';
  const value = Math.max(0, Math.floor(seconds));
  return `${String(Math.floor(value / 60)).padStart(2, '0')}:${String(value % 60).padStart(2, '0')}`;
}
function proPlayerButton(player, selected, opponent = false, onCourt = false) {
  const status = opponent ? '' : onCourt ? 'on-court' : 'bench';
  return `<button class="pro-player ${status} ${selected ? 'selected' : ''}" data-action="${opponent ? 'pro-select-opponent' : 'pro-select-player'}" data-id="${esc(player.id)}" aria-pressed="${selected ? 'true' : 'false'}"><strong>${esc(player.number)}</strong><span>${esc(player.name)}</span></button>`;
}
function proCourtHTML(g, events, interactive = false) {
  ...
  return `<svg class="pro-court" viewBox="0 0 940 500" ${interaction} aria-label="${interactive ? 'フルコート。シュート位置をタップ' : 'フルコート'}"><rect class="pro-court-surface" x="24" y="24" width="892" height="452" rx="2"></rect><g class="pro-court-markings">${PRO_COURT_MARKINGS}</g><g class="pro-shot-markers">${markers}</g></svg>`;
}
export function proLiveView(s, g, events, clockSeconds = null) {
  ...
  const ownPlayers = [...g.roster].sort((left, right) => Number(onCourt.has(right.id)) - Number(onCourt.has(left.id))).map(player => proPlayerButton(player, selection.playerId === player.id || s.proSub?.outPlayerId === player.id, false, onCourt.has(player.id)));
