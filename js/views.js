...
const opponentRosterNumbers = value => String(value || '').split(/\r?\n/).map(line => {
  const trimmed = line.trim();
  const match = trimmed.match(/^(\d{1,3})(?:\s+|,|$)/);
  return match ? match[1] : /^\d{1,3}$/.test(trimmed) ? trimmed : '';
}).filter(Boolean);
const OPPONENT_ROSTER_SLOTS = 12;
const opponentNumberOptions = selected => {
  const numbers = new Set(Array.from({ length: 100 }, (_, number) => String(number)));
  if (selected) numbers.add(String(selected));
  return [...numbers].sort((left, right) => Number(left) - Number(right)).map(number => `<option value="${number}" ${number === String(selected || '') ? 'selected' : ''}>${number}</option>`).join('');
};
const opponentRosterPickerHTML = numbers => `<div class="opponent-number-picker" role="group" aria-label="相手選手の背番号">${Array.from({ length: Math.max(OPPONENT_ROSTER_SLOTS, numbers.length) }, (_, index) => `<label><span>相手${index + 1}</span><select name="opponentRosterNumber" aria-label="相手選手${index + 1}の背番号"><option value="">—</option>${opponentNumberOptions(numbers[index] || '')}</select></label>`).join('')}</div>`;
export function gameFormView(s, d) {
  ...
  const opponentNumbers = opponentRosterNumbers(d.opponentRosterText);
...
function proCourtHTML(g, events, interactive = false, selectionSide = null) {
  ...
  const backcourt = !halfCourt && interactive && selectionSide ? shotDirection === 'right'
    ? '<rect class="pro-backcourt-overlay" data-action="pro-backcourt" x="24" y="10" width="446" height="480" aria-label="バックコート。選択不可"><title>BACK COURT · 選択不可</title></rect><text class="pro-backcourt-label" x="247" y="250" text-anchor="middle">BACK COURT</text>'
    : '<rect class="pro-backcourt-overlay" data-action="pro-backcourt" x="470" y="10" width="446" height="480" aria-label="バックコート。選択不可"><title>BACK COURT · 選択不可</title></rect><text class="pro-backcourt-label" x="693" y="250" text-anchor="middle">BACK COURT</text>'
    : '';
  const viewBox = halfCourt ? '0 0 500 500' : '0 0 940 500';
  const surface = halfCourt ? '<rect class="pro-court-surface" x="10" y="24" width="480" height="452" rx="2"></rect>' : '<rect class="pro-court-surface" x="24" y="10" width="892" height="480" rx="2"></rect>';
...
export function proLiveView(s, g, events, clockSeconds = null) {
  ...
  const opponentPlayers = [...(g.opponentRoster || [])].sort((left, right) => Number(opponentOnCourt.has(right.id)) - Number(opponentOnCourt.has(left.id))).map(player => proPlayerButton(player, opponentSelection.playerId === player.id || s.proOpponentSub?.outPlayerId === player.id, true, opponentOnCourt.has(player.id), a.opponentPlayers[player.id]));
  ...
  const hint = s.proSub ? (s.proSub.outPlayerId ? '交代するベンチの選手をタップ' : '交代するコート上の選手をタップ') : s.proOpponentSub ? (s.proOpponentSub.outPlayerId ? '交代するベンチの相手選手をタップ' : '交代するコート上の相手選手をタップ') : ownPointReady ? 'コート上をタップして位置を記録' : opponentPointReady ? 'コート上をタップして相手の位置を記録' : selection.type || opponentSelection.type ? '選手をタップ' : 'プレーを選択';
...
export function boxView(s, g, events) {
  ...
  const opponentRow = (p, st) => `<tr><th scope="row"><b>${esc(p.number)}</b></th>${BOX_STAT_KEYS.map(k => `<td ${k === 'PTS' ? 'class="pts-cell"' : ''}>${st[k]}</td>`).join('')}</tr>`;
...
function fullCourtShotChartMapHTML(shots, playerId) {
  ...
  return `<svg class="shot-court-map shot-chart-map pro-shot-chart-map" viewBox="0 0 940 500" role="img" aria-label="ショットチャート。○×で各シュートの成功・失敗を表示"><rect class="pro-chart-court-surface" x="24" y="10" width="892" height="480"></rect><g class="pro-chart-court-markings">${PRO_COURT_MARKINGS}</g><g class="pro-shot-chart-markers">${markerHTML}</g></svg><div class="pro-shot-zone-summary">${summary}</div>`;
}
