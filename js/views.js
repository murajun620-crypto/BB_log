import { STATS, STAT_DEFS, SHOT_ZONES, PRO_COURT, aggregate, aggregateGames, activeEvents, attackDirectionForPeriod, efficiency, eventLabel, formatGame, fullCourtPointFromHalf, halfCourtPointFromFull, isShotEvent, lineup, localDate, opponentLineup, oppositeDirection, percent, shotDirectionForEvent, shotZoneForEvent, shotZoneFromPoint, shotZoneLabel } from './domain.js';
...
const jerseyNumberForSort = player => {
  const value = String(player?.number ?? '').trim();
  return /^[0-9]+$/.test(value) ? Number(value) : Number.POSITIVE_INFINITY;
};
export const sortPlayersByNumber = players => players
  .map((player, index) => ({ player, index }))
  .sort((a, b) => jerseyNumberForSort(a.player) - jerseyNumberForSort(b.player) || a.index - b.index)
  .map(({ player }) => player);
export function teamFormView(s, draft) {
  const displayPlayers = sortPlayersByNumber(draft.players);
  return shell(s, `${heading('TEAM SETUP', draft.id ? 'チームを編集' : 'チームを登録')}<form id="team-form"><div class="panel"><label>チーム名<input name="name" maxlength="40" required placeholder="例：TOKYO HOOPS" value="${esc(draft.name)}"></label></div><div class="section-heading"><h2>選手 <span class="muted">${draft.players.length}人</span></h2><span class="draft-status">下書きを自動保存</span></div><div class="roster-editor">${displayPlayers.map((p, i) => `<div class="roster-edit-row" data-player-id="${esc(p.id)}"><label><span>${i === 0 ? '背番号' : '<span class="sr-only">背番号</span>'}</span><input name="number" data-jersey-number aria-label="選手${i + 1}の背番号" inputmode="numeric" pattern="[0-9]{1,3}" maxlength="3" required placeholder="00" value="${esc(p.number)}"></label><label><span>${i === 0 ? '名前' : '<span class="sr-only">名前</span>'}</span><input name="playerName" aria-label="選手${i + 1}の名前" maxlength="40" required placeholder="選手名" value="${esc(p.name)}"></label><button type="button" class="icon-button remove-player" data-action="remove-player" data-id="${esc(p.id)}" aria-label="選手${i + 1}を外す">${icon('close')}</button></div>`).join('')}</div>${action('add-player', `${icon('plus')}選手を追加`, 'button secondary full', 'type="button"')}<p class="help">先発5人は試合作成時に選択できます。登録内容の変更は、過去の試合には影響しません。</p><div class="form-actions"><a href="#teams" class="button secondary">戻る</a><button class="button primary" type="submit">チームを保存 ${icon('check')}</button></div></form>`);
}
const legacyOpponentRosterColumns = text => {
  const rows = String(text ?? '').split(/\r?\n/).map(line => line.trim()).filter(Boolean).map(line => {
    const match = line.match(/^([0-9]{1,3})(?:[ \t]+(.+?))?$/);
    return { number: match?.[1] || '', name: match?.[2]?.trim() || '' };
  });
  return { numbers: rows.map(row => row.number).join('\n'), names: rows.map(row => row.name).join('\n') };
};
const opponentRosterColumns = roster => {
  if (Array.isArray(roster)) return { numbers: roster.map(player => player.number || '').join('\n'), names: roster.map(player => player.name || '').join('\n') };
  if (roster && typeof roster === 'object') {
    const numbers = roster.numbersText ?? roster.numbers;
    const names = roster.namesText ?? roster.names;
    const legacy = roster.legacyText ?? roster.text;
    if ((String(numbers ?? '').trim() || String(names ?? '').trim()) || !legacy) return { numbers: String(numbers ?? ''), names: String(names ?? '') };
    return legacyOpponentRosterColumns(legacy);
  }
  return legacyOpponentRosterColumns(roster);
};
const opponentRosterRows = roster => {
  const { numbers, names } = opponentRosterColumns(roster);
  const numberLines = String(numbers ?? '').replace(/\r/g, '').split('\n');
  const nameLines = String(names ?? '').replace(/\r/g, '').split('\n');
  const count = Math.max(numberLines.length, nameLines.length, 1);
  return Array.from({ length: count }, (_, index) => ({ number: numberLines[index]?.trim() || '', name: nameLines[index]?.trim() || '' }));
};
const opponentRosterRowHTML = (row, index) => `<div class="roster-edit-row opponent-roster-edit-row" data-opponent-roster-row><label><span>${index === 0 ? '背番号' : '<span class="sr-only">背番号</span>'}</span><input name="opponentRosterNumber" data-opponent-roster-number data-jersey-number aria-label="相手選手${index + 1}の背番号" inputmode="numeric" pattern="[0-9]{1,3}" maxlength="3" placeholder="00" value="${esc(row.number)}"></label><label><span>${index === 0 ? '名前' : '<span class="sr-only">名前</span>'}</span><input name="opponentRosterName" aria-label="相手選手${index + 1}の名前（任意）" maxlength="40" placeholder="選手名（任意）" value="${esc(row.name)}"></label><button type="button" class="icon-button remove-player" data-action="remove-opponent-roster-player" aria-label="相手選手${index + 1}を削除">${icon('close')}</button></div>`;
const opponentRosterInputHTML = (roster, className = 'span-2') => {
  const rows = opponentRosterRows(roster);
  return `<div class="${className} opponent-roster-input"><span class="field-label">相手選手（1人ずつ登録）</span><div class="roster-editor opponent-roster-editor" data-opponent-roster-editor>${rows.map(opponentRosterRowHTML).join('')}</div>${action('add-opponent-roster-player', `${icon('plus')}相手選手を追加`, 'button secondary full', 'type="button"')}<small class="field-help">選手ごとに背番号と名前を入力してください。背番号は半角数字1〜3桁、名前は任意です。不要な行は右の×で削除できます。交代を使う場合はベンチ選手も入力してください。</small></div>`;
};
export function gameFormView(s, d) {
  ...
  const displayPlayers = sortPlayersByNumber(team.players);
...
export function liveSettingsHTML(g) {
  ...
  return `<form id="live-settings-form"><p class="help">試合中でも試合日・記録モード・Proの入力設定を変更できます。保存後すぐにLIVE画面へ戻ります。</p><label>試合日<input type="date" name="date" required value="${esc(g.date || localDate())}"></label><label class="spaced">記録モード<div class="segmented">${radio('mode', 'standard', '標準', mode === 'standard')}${radio('mode', 'pro', 'Pro', mode === 'pro')}</div></label><div class="live-settings-pro-options"><label class="spaced">ゲームクロック<div class="segmented">${radio('clockEnabled', 'off', '使わない', !clockEnabled)}${radio('clockEnabled', 'on', '使う', clockEnabled)}</div></label><label class="spaced">相手チームの記録<div class="segmented">${radio('opponentTracking', 'score', '総得点のみ', tracking === 'score')}${radio('opponentTracking', 'player', '個人も記録', tracking === 'player')}</div></label>${opponentRosterInputHTML(g.opponentRoster, 'spaced')}</div><button class="button primary full spaced" type="submit">設定を保存</button></form>`;
}
...
// Court markings deliberately contain only actual court lines. The corner/wing
// split exists in the classifier and coloured area map, not as a LIVE-court grid.
const PRO_COURT_MAIN_LINES = [
  '<path d="',
  `M${PRO_COURT.courtLeftX} 0H${PRO_COURT.courtRightX}V${PRO_COURT.height}H${PRO_COURT.courtLeftX}Z`,
  `M${PRO_COURT.centerX} 0V${PRO_COURT.height}`,
  `M${PRO_COURT.courtLeftX} ${PRO_COURT.threeLineCornerY}H${PRO_COURT.leftThreeArcEndpointX}`,
  `M${PRO_COURT.courtLeftX} ${PRO_COURT.height - PRO_COURT.threeLineCornerY}H${PRO_COURT.leftThreeArcEndpointX}`,
  `M${PRO_COURT.leftThreeArcEndpointX} ${PRO_COURT.threeLineCornerY}A${PRO_COURT.threeRadius} ${PRO_COURT.threeRadius} 0 0 1 ${PRO_COURT.leftThreeArcEndpointX} ${PRO_COURT.height - PRO_COURT.threeLineCornerY}`,
  `M${PRO_COURT.courtRightX} ${PRO_COURT.threeLineCornerY}H${PRO_COURT.rightThreeArcEndpointX}`,
  `M${PRO_COURT.courtRightX} ${PRO_COURT.height - PRO_COURT.threeLineCornerY}H${PRO_COURT.rightThreeArcEndpointX}`,
  `M${PRO_COURT.rightThreeArcEndpointX} ${PRO_COURT.threeLineCornerY}A${PRO_COURT.threeRadius} ${PRO_COURT.threeRadius} 0 0 0 ${PRO_COURT.rightThreeArcEndpointX} ${PRO_COURT.height - PRO_COURT.threeLineCornerY}`,
  `M${PRO_COURT.courtLeftX} ${PRO_COURT.paintTop}H${PRO_COURT.leftFreeThrowX}V${PRO_COURT.paintBottom}H${PRO_COURT.courtLeftX}`,
  `M${PRO_COURT.courtRightX} ${PRO_COURT.paintTop}H${PRO_COURT.rightFreeThrowX}V${PRO_COURT.paintBottom}H${PRO_COURT.courtRightX}`,
  'M54 218V282M886 218V282M162 176V184M186 176V184M162 324V316M186 324V316M778 176V184M754 176V184M778 324V316M754 324V316',
  'M210 196A54 54 0 0 1 210 304M730 196A54 54 0 0 0 730 304',
  'M74 213A37 37 0 0 1 74 287M866 213A37 37 0 0 0 866 287',
  '"></path><circle cx="470" cy="250" r="50"></circle><circle cx="74" cy="250" r="10"></circle><circle cx="866" cy="250" r="10"></circle>',
].join('');
...
const isPortraitViewport = () => {
  ...
  return media ? media.matches : window.innerHeight >= window.innerWidth;
};
const isIPhonePortraitViewport = () => isIPhonePortrait(typeof navigator === 'undefined' ? '' : navigator.userAgent, isPortraitViewport());
...
function proShotAreaFeedbackHTML(feedback, halfCourt, attackDirection) {
  ...
  const point = halfCourt
    ? halfCourtPointFromFull(feedback.shotX, feedback.shotY, markerDirection)
    : { x: feedback.shotX * 940, y: feedback.shotY * 500 };
  const zone = shotZoneForEvent(feedback, feedback.shotDirection);
  const label = shotZoneLabel(zone);
  if (!point || !label) return '';
  const width = halfCourt ? 500 : 940;
  const labelWidth = halfCourt ? 188 : 184;
  const halfLabelWidth = labelWidth / 2;
  const labelX = Math.max(halfLabelWidth + 8, Math.min(width - halfLabelWidth - 8, point.x));
  const labelY = point.y < 160 ? Math.min(472, point.y + 58) : Math.max(28, point.y - 58);
...
function shotTypeLabel(shot) {
  ...
  if (shot?.eventType?.startsWith('FT')) return 'FT';
...
export function shotMarkerDetailFeedbackHTML({ player = '選手不明', area = '位置不明', result = '', points = '', x = 0, y = 0, width = 940, height = 500 } = {}) {
  ...
  const playerLabel = player || '選手不明';
  const areaLabel = area || '位置不明';
  const labelWidth = Math.min(184, Math.max(144, Math.max(String(playerLabel).length, String(areaLabel).length) * 14 + 28));
...
function proCourtHTML(g, events, interactive = false, selectionSide = null, shotFeedback = null) {
  ...
  const markers = activeEvents(events).filter(event => isShotEvent(event) && Number.isFinite(event.shotX) && Number.isFinite(event.shotY)).map(event => {
    const eventAttackDirection = attackDirectionForPeriod(g, event.periodId);
    const markerDirection = event.side === 'opponent' ? oppositeDirection(eventAttackDirection) : eventAttackDirection;
    ...
    const area = shotZoneLabel(shotZoneForEvent(event, markerDirection)) || '位置不明';
    const result = made ? '○ 成功' : '× 失敗';
    const points = shotTypeLabel(event) || event.eventType || '';
    const markerLabel = `${label}。タップで詳細、長押し／3Dタッチで編集`;
    return `<g class="pro-shot-marker ${made ? 'made' : 'miss'}${interactive ? ' pro-shot-marker-selecting' : ''}" data-shot-marker="pro-live" data-shot-editable="true" data-event-id="${esc(event.id)}" data-shot-game-id="${esc(event.gameId)}" data-shot-side="${esc(event.side || 'home')}" data-shot-player="${esc(player)}" data-shot-area="${esc(area)}" data-shot-result="${esc(result)}" data-shot-points="${esc(points)}" data-shot-x="${x.toFixed(1)}" data-shot-y="${y.toFixed(1)}" data-shot-view-width="${halfCourt ? '500' : '940'}" data-shot-view-height="500" role="button" tabindex="0" aria-label="${esc(markerLabel)}" transform="translate(${x.toFixed(1)} ${y.toFixed(1)})"><circle class="shot-marker-hit-area" r="24"></circle><text text-anchor="middle" dy=".36em">${made ? '○' : '×'}</text><title>${esc(markerLabel)}</title></g>`;
  }).join('');
  ...
  const backcourt = !halfCourt && interactive && selectionSide ? shotDirection === 'right'
    ? '<rect class="pro-backcourt-overlay" data-action="pro-backcourt" x="24" y="0" width="446" height="500" aria-label="バックコート。選択不可"><title>BACK COURT · 選択不可</title></rect><text class="pro-backcourt-label" x="247" y="250" text-anchor="middle">BACK COURT</text>'
    : '<rect class="pro-backcourt-overlay" data-action="pro-backcourt" x="470" y="0" width="446" height="500" aria-label="バックコート。選択不可"><title>BACK COURT · 選択不可</title></rect><text class="pro-backcourt-label" x="693" y="250" text-anchor="middle">BACK COURT</text>'
    : '';
  const viewBox = halfCourt ? '0 0 500 500' : '0 0 940 500';
  const surface = halfCourt ? '<rect class="pro-court-surface" x="0" y="24" width="500" height="452" rx="2"></rect>' : '<rect class="pro-court-surface" x="24" y="0" width="892" height="500" rx="2"></rect>';
  ...
  return `<svg class="pro-court${halfCourt ? ' pro-court-half' : ''}" viewBox="${viewBox}" ${interaction} aria-label="${label}">${surface}${backcourt}<g class="pro-court-markings">${markings}</g><g class="pro-shot-markers">${markers}</g>${proShotAreaFeedbackHTML(shotFeedback, halfCourt, attackDirection)}</svg>`;
}
export function proLiveView(s, g, events, clockSeconds = null) {
  const a = aggregate(g, events);
  const period = g.periods.find(p => p.id === g.currentPeriodId);
  const halfCourt = isIPhonePortraitViewport();
  const opponentShotPointAllowed = !halfCourt;
  const attackDirection = attackDirectionForPeriod(g);
  ...
  const opponentSide = g.opponentTracking === 'player'
    ? `<p class="pro-roster-label">OPPONENT</p><div class="pro-roster" aria-label="相手選手一覧">${opponentRoster}</div><button class="pro-sub-button opponent-sub-button" data-action="pro-opponent-sub" aria-label="相手チームの交代">相手交代</button><div class="pro-action-row pro-opponent-actions">${opponentActions.map(opponentAction).join('')}</div>${opponentShotNote}`
    : `<p class="pro-roster-label">OPPONENT · SCORE</p><div class="pro-opponent-score">${[1, 2, 3].map(points => action('opponent', `+${points}`, 'opponent-button', `data-points="${points}" aria-label="相手に${points}点追加"`)).join('')}</div>`;
  const directionButton = halfCourt ? '' : action('toggle-pro-attack', `<span>攻撃方向</span><b>${attackArrow} ${attackGoal}</b><small>${g.format === 'quarters' ? 'ハーフタイムで自動反転' : 'タップで変更'}</small>`, 'pro-direction-button', `aria-label="自チームの攻撃は${attackGoal}です。タップで変更"`);
  return `<main class="pro-live-screen ${step}" data-pro-step="${step}"><header class="pro-header"><a href="#home" class="icon-button" aria-label="ホームに戻る">${icon('back')}</a><span class="live-title"><span class="live-dot"></span>PRO LIVE</span>${cancelAction}<span class="save-state">${icon('check')}保存済み</span>${action('game-menu', icon('more'), 'icon-button', 'aria-label="試合メニュー"')}</header><section class="pro-scoreboard" aria-label="スコア"><div><span>MY TEAM</span><strong>${esc(g.teamName)}</strong></div><b class="pro-score-digits">${digitalText(a.team.PTS)}<i>–</i>${digitalText(a.opponent)}</b><div><span>OPPONENT</span><strong>${esc(g.opponentName)}</strong></div></section><div class="pro-meta"><span class="pro-period">${action('period-menu', `${esc(period.label)} ⌄`, 'period-button', 'aria-label="ピリオド操作"')}</span><span>${esc(formatGame(g))}</span>${g.clockEnabled ? `<button class="pro-clock ${g.clockRunning ? 'running' : ''}" data-action="pro-clock-edit" aria-label="ゲームクロックを編集"><span id="pro-clock-value" class="pro-clock-digits">${digitalText(clockText(clockSeconds))}</span><small>EDIT</small></button><button class="pro-clock-toggle ${g.clockRunning ? 'running' : ''}" data-action="pro-clock-toggle" aria-label="ゲームクロックを${g.clockRunning ? '停止' : '開始'}"><small>${g.clockRunning ? 'STOP' : 'START'}</small></button><button class="pro-clock-reset" data-action="pro-clock-reset" aria-label="ゲームクロックをリセット">↺</button>` : '<span class="pro-clock-off">CLOCK OFF</span>'}</div><div class="pro-layout"><aside class="pro-side pro-home-side"><p class="pro-roster-label">MY TEAM</p><div class="pro-roster-key"><span class="on-court-key">コート上</span><span class="bench-key">ベンチ</span></div><div class="pro-roster" aria-label="自チーム選手一覧">${ownRoster}</div><button class="pro-sub-button" data-action="pro-sub" aria-label="自チームの交代">交代</button></aside><section class="pro-center"><div class="pro-court-status">${selectedStatus}</div><div class="pro-court-wrap">${proCourtHTML(g, events, ownPointReady || opponentPointReady, ownPointReady ? 'home' : opponentPointReady ? 'opponent' : null, s.proShotFeedback?.gameId === g.id ? s.proShotFeedback : null)}${directionButton}</div><div class="pro-action-panel"><div class="pro-action-row">${PRO_SHOT_ACTIONS.map(actionButton).join('')}</div><div class="pro-action-row pro-other-row">${PRO_OTHER_ACTIONS.map(otherButton).join('')}</div></div></section><aside class="pro-side pro-away-side">${opponentSide}</aside></div><footer class="pro-footer">${action('undo', `${icon('undo')}<strong>UNDO</strong>`, 'undo-button', activeEvents(events).length ? '' : 'disabled')}${action('events', `${icon('history')}<span>履歴</span>`, 'pro-history-button') }<a href="#box/${g.id}" class="box-button">${icon('history')}<span>BOX SCORE</span></a></footer></main>`;
}
...
export function strategyBoardItemsHTML(board = {}) {
  return strategyBoardItems(board).map(item => {
    ...
    if (item.kind === 'athlete' || item.kind === 'marker' && item.marker === 'player') {
      ...
      return `<g class="strategy-board-uniform strategy-board-${team}" data-strategy-item="${esc(item.id)}" data-strategy-movable="true" role="button" aria-label="${team === 'away' ? '相手' : '味方'} ${esc(item.label || '')}。ドラッグで移動" transform="translate(${x} ${y})"><path d="M-13-24-25-17-37-4-26 13-17 7-17 26H17V7L26 13 37-4 25-17 13-24Z"></path><path class="strategy-board-uniform-collar" d="M-10-23Q0-12 10-23"></path><text y="8" text-anchor="middle">${esc(item.label || '')}</text></g>`;
    }
    if (item.kind === 'ball' || item.kind === 'marker' && item.marker === 'ball') return `<g class="strategy-board-ball" data-strategy-item="${esc(item.id)}" data-strategy-movable="true" role="button" aria-label="ボール。ドラッグで移動" transform="translate(${x} ${y})"><circle r="18"></circle><path d="M-18 0H18M0-18V18M-13-13C-4-7 4 7 13 13M13-13C4-7-4 7-13 13"></path></g>`;
...
export function strategyBoardHTML(board = {}) {
  ...
  const toolButton = ([tool, label]) => `<button type="button" class="strategy-tool ${selected === tool ? 'active' : ''}" data-action="strategy-tool" data-tool="${tool}" aria-pressed="${selected === tool}">${label}</button>`;
  const placeTools = Object.entries(strategyBoardToolLabels).filter(([tool]) => ['home', 'away', 'ball'].includes(tool)).map(toolButton).join('');
  const drawTools = Object.entries(strategyBoardToolLabels).filter(([tool]) => ['line', 'arrow', 'erase'].includes(tool)).map(toolButton).join('');
  return `<div class="strategy-board-layout"><aside class="strategy-board-side strategy-board-side-left"><div class="strategy-board-tools" role="group" aria-label="作戦ボードの道具：配置">${placeTools}</div><div class="strategy-board-counts" data-strategy-board-counts aria-live="polite">${strategyBoardCountsHTML(board)}</div></aside><div class="strategy-board-stage"><svg class="strategy-board-canvas" data-strategy-board viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid meet" role="img" aria-label="作戦ボード。味方と相手を各5人まで、ボールを1個配置し、ラインと矢印を描けます。配置済みの選手とボールはドラッグで移動できます"><defs><marker id="strategy-board-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"></path></marker></defs><rect class="strategy-board-surface" x="20" y="20" width="960" height="560" rx="4"></rect><g class="strategy-board-court"><path d="M20 20H980V580H20ZM500 20V580M20 88H194M20 512H194M194 88A255 255 0 0 1 194 512M980 88H806M980 512H806M806 88A255 255 0 0 0 806 512M20 230H250V370H20M980 230H750V370H980M250 250A60 60 0 0 1 250 350M750 250A60 60 0 0 0 750 350M96 270V330M904 270V330M250 230A42 42 0 0 1 250 370M750 230A42 42 0 0 0 750 370"></path><circle cx="500" cy="300" r="56"></circle><circle cx="96" cy="300" r="12"></circle><circle cx="904" cy="300" r="12"></circle></g><g data-strategy-board-items>${strategyBoardItemsHTML(board)}</g><g data-strategy-board-draft></g></svg></div><aside class="strategy-board-side strategy-board-side-right"><div class="strategy-board-tools" role="group" aria-label="作戦ボードの道具：描画">${drawTools}</div></aside></div><div class="strategy-board-bottom"><p class="strategy-board-hint" id="strategy-board-hint">味方を選び、コートをタップして配置。配置済みの選手とボールはドラッグで移動</p><div class="strategy-board-actions"><button type="button" class="strategy-board-action" data-action="strategy-undo">UNDO</button><button type="button" class="strategy-board-action danger" data-action="strategy-clear">全消去</button></div></div><div class="strategy-board-footnote"><span>全画面で利用できます。ボード内容はこの端末の試合画面だけに保持されます。</span></div>`;
}
const shotType = type => ['2PM', '2PX', '3PM', '3PX'].includes(type);
export const BOX_STAT_KEYS = ['PTS', 'EFF', 'REB', 'AST', 'STL', 'BLK', 'TO', 'PF', 'FD'];
export const DETAIL_STAT_KEYS = ['OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TO', 'PF', 'FD', 'EFF'];
export const statLabel = key => ({ OREB: 'OR', DREB: 'DR', PF: 'F', FD: 'FD' }[key] || key);
export const statValue = (stats, key) => key === 'EFF' ? efficiency(stats) : stats[key];
export function shotChartHTML(events, playerId = null, displayMode = 'points', players = [], editable = false, games = []) {
  const gameById = new Map((Array.isArray(games) ? games : [games]).filter(Boolean).map(game => [game.id, game]));
  const shots = activeEvents(events).filter(event => shotType(event.eventType) && event.side !== 'opponent' && (!playerId || event.playerId === playerId)).map(event => ({ ...event, shotDirection: shotDirectionForEvent(gameById.get(event.gameId), event) })).filter(event => shotZoneForEvent(event));
  if (!shots.length) return '';
  return `<section class="shot-chart"><div class="section-heading"><h2>ショットチャート</h2><span class="muted">成功数/試投数・成功率</span></div>${shotChartMapHTML(shots, playerId, displayMode, players, editable)}</section>`;
}
...
export function boxView(s, g, events) {
  ...
  const row = (p, st, total = false) => `<tr ${total ? 'class="total-row"' : ''}><th scope="row">${total ? 'TEAM' : `<button data-action="player-detail" data-id="${p.id}"><b>${esc(p.number)}</b><span>${esc(p.name)}</span></button>`}</th>${BOX_STAT_KEYS.map(k => `<td ${k === 'PTS' ? 'class="pts-cell"' : ''}>${statValue(st, k)}</td>`).join('')}</tr>`;
  const opponentRow = (p, st) => `<tr><th scope="row"><b>${esc(p.number)}</b>${p.name ? `<span>${esc(p.name)}</span>` : ''}</th>${BOX_STAT_KEYS.map(k => `<td ${k === 'PTS' ? 'class="pts-cell"' : ''}>${statValue(st, k)}</td>`).join('')}</tr>`;
  const opponentBox = g.mode === 'pro' && g.opponentTracking === 'player' && g.opponentRoster?.length ? `<div class="section-heading"><h2>相手選手スタッツ</h2></div><div class="box-table-wrap"><table class="box-table"><thead><tr><th scope="col">PLAYER</th>${BOX_STAT_KEYS.map(k => `<th scope="col">${label(k)}</th>`).join('')}</tr></thead><tbody>${sortPlayersByNumber(g.opponentRoster).map(p => opponentRow(p, a.opponentPlayers[p.id])).join('')}<tr class="total-row"><th scope="row">TEAM</th>${BOX_STAT_KEYS.map(k => `<td>${statValue(a.opponentTeam, k)}</td>`).join('')}</tr></tbody></table></div>` : '';
...
export function sharedPlayerDetail(report, id) {
  ...
  return `<p class="shared-game-context">${esc(report.teamName)} vs. ${esc(report.opponentName)} · ${esc(report.date.replaceAll('-', '.'))}</p><div class="player-detail-name"><span class="jersey">${esc(player.number)}</span><h3>${esc(player.name)}</h3><div class="detail-points"><b>${stats.PTS}</b> PTS</div></div>${shooting(stats)}<div class="detail-stats">${DETAIL_STAT_KEYS.map(key => `<div><span>${statLabel(key)}</span><strong>${statValue(stats, key)}</strong></div>`).join('')}</div>`;
}
...
export function aggregateView(s, games, events) {
  ...
  const label = key => ({ OREB: 'OR', DREB: 'DR', PF: 'F' }[key] || key);
  const cell = (stats, key = '') => { const value = statValue(stats, key); return `<td ${key === 'PTS' ? 'class="pts-cell"' : ''}>${mode === 'average' ? average(value, games.length) : value}</td>`; };
...
export function aggregateViewUnified(s, games, events) {
  ...
  const modeText = mode === 'average' ? '平均' : '合計';
  const label = key => ({ OREB: 'OR', DREB: 'DR', PF: 'F' }[key] || key);
  const cell = (stats, key) => `<td${key === 'PTS' ? ' class="pts-cell"' : ''}>${mode === 'average' ? average(statValue(stats, key), games.length) : statValue(stats, key)}</td>`;
  const row = player => `<tr><th scope="row"><button data-action="aggregate-player-detail" data-id="${esc(player.id)}"><b>${esc(player.number)}</b><span>${esc(player.name)}</span></button></th>${BOX_STAT_KEYS.map(key => cell(player.stats, key)).join('')}</tr>`;
  const gameList = games.map(game => {
    const score = aggregate(game, events.filter(event => event.gameId === game.id));
...
export function aggregatePlayerDetailWithGames(report, id, mode = 'total', games = [], events = [], selected = 'total') {
  ...
  return `${options}<p class="shared-game-context">${esc(context)}</p><div class="player-detail-name"><span class="jersey">${esc(basePlayer.number)}</span><h3>${esc(basePlayer.name)}</h3><div class="detail-points"><b>${value(playerStats.PTS)}</b> PTS</div></div>${shooting(playerStats, averageMode ? games.length : selectedDetail ? 1 : games.length)}<div class="detail-stats">${DETAIL_STAT_KEYS.map(key => `<div><span>${statLabel(key)}</span><strong>${value(statValue(playerStats, key))}</strong></div>`).join('')}</div>`;
}
export function settingsView(s) {
  const preferenceRows = `<label class="setting-row"><span><strong>連続入力</strong><small>シュート後にAST / OR・DRの入力を続けて提案</small></span><input type="checkbox" role="switch" id="continuous" ${s.preferences.continuous ? 'checked' : ''}></label><label class="setting-row"><span><strong>Advancedモード</strong><small>2P・3Pのシュート位置を記録します。</small></span><input type="checkbox" role="switch" id="advancedMode" ${s.preferences.advancedMode ? 'checked' : ''}></label><label class="setting-row"><span><strong>試合中は画面をスリープさせない</strong><small>試合画面を開いている間だけ自動ロックを防ぎます。電池を消費します。</small></span><input type="checkbox" role="switch" id="keepAwake" ${s.preferences.keepAwake ? 'checked' : ''}></label><label class="setting-row"><span><strong>外観</strong><small>見やすい明るさを選択</small></span><select id="theme" aria-label="外観">${[['system', '端末に合わせる'], ['light', 'ライト'], ['dark', 'ダーク']].map(([v, label]) => `<option value="${v}" ${s.preferences.theme === v ? 'selected' : ''}>${label}</option>`).join('')}</select></label>`;
  return shell(s, `${heading('PREFERENCES', '設定とデータ')}<section class="panel settings-panel"><h2>試合入力・表示</h2>${preferenceRows}</section><div data-cloud-settings-slot></div><section class="panel settings-panel"><h2>受け取った共有レポート</h2><p class="help">共有された1試合のファイルを、データへ取り込まず読み取り専用で開きます。</p><label class="button primary full" for="shared-report-file">${icon('share')}共有レポートを開く</label><input type="file" id="shared-report-file" accept=".json,application/json" class="sr-only"></section><section class="panel settings-panel"><h2>バックアップ</h2><p class="help">試合・チーム・設定を1つのJSONに保存します。定期的に「ファイル」などへ書き出してください。</p>${action('export-json', `${icon('download')}全データを書き出す`, 'button primary full')}<label class="button secondary full spaced" for="restore-file">JSONから復元</label><input type="file" id="restore-file" accept=".json,application/json" class="sr-only"><p class="help">復元前に内容を検証し、件数を表示します。復元すると現在の全データが置き換わります。</p></section><section class="panel settings-panel"><h2>この端末の保存状態</h2><div class="storage-status"><span>${s.data.teams.length}チーム / ${s.data.games.length}試合 / ${activeEvents(s.data.events).length}記録</span></div><p class="help">${s.pwa.error ? esc(s.pwa.error) : s.pwa.ready ? 'アプリ本体のキャッシュが完了しました。通信がなくても利用できます。' : '初回のキャッシュ完了までオンラインでお待ちください。'}</p>${s.pwa.update ? '<p class="notice">更新があります。下のボタンから適用できます。</p>' + action('apply-update', '最新版に更新', 'button primary full') : action('check-update', '最新版を確認', 'button secondary full')}${action('persist', '保存領域の保持をリクエスト', 'button secondary full')}<p class="help" id="persist-status">ブラウザのデータ削除や端末の故障に備え、JSONバックアップもご利用ください。</p></section><details class="settings-help"><summary>iPhoneのホーム画面に追加</summary><ol class="install-steps"><li>Safariでこのアプリを開く</li><li>共有メニューから「ホーム画面に追加」</li><li>表示される場合は「Webアプリとして開く」をONにし、追加</li><li>追加したアプリを起動し、「オフライン利用OK」を確認</li></ol></details><p class="version-note">COURTSIDE ${APP_VERSION} · BUILT FOR THE SIDELINES</p>`);
}
...
function buildHalfCourtShotZones() {
  ...
  for (let y = HALF_COURT_ZONE_BOUNDS.top; y < HALF_COURT_ZONE_BOUNDS.bottom; y += HALF_COURT_ZONE_GRID) {
    ...
    for (let x = HALF_COURT_ZONE_BOUNDS.left; x < HALF_COURT_ZONE_BOUNDS.right; x += HALF_COURT_ZONE_GRID) {
      ...
      const zoneId = shotZoneFromPoint(null, point.x, point.y, 'left');
...
function fullCourtShotChartMapHTML(shots, playerId, players = [], editable = false) {
...
export function shotChartMapHTML(shots = [], playerId = null, displayMode = 'points', players = [], editable = false) {
...
export function editEventHTML(g, e) {
  const zoneField = e.eventType && shotType(e.eventType) ? `<label class="spaced">シュート位置<select name="shotZone"><option value="">位置なし</option>${SHOT_ZONES.map(zone => `<option value="${zone.id}" ${zone.id === shotZoneForEvent(e, shotDirectionForEvent(g, e)) ? 'selected' : ''}>${esc(zone.label)}</option>`).join('')}</select></label>` : '';
