...
const opponentRosterText = roster => (roster || []).map(player => [player.number, player.name].filter(Boolean).join(' ')).join('\n');
const opponentRosterInputHTML = (text, className = 'span-2') => `<label class="${className}"><span class="field-label">相手選手（1行に1人：背番号 名前）</span><textarea name="opponentRosterText" rows="5" placeholder="4&#10;7 相手選手">${esc(text)}</textarea><small class="field-help">背番号は半角数字1〜3桁。名前は任意です。空行は無視されます。交代を使う場合はベンチ選手も入力してください。</small></label>`;
