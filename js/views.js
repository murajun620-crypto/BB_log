...
const opponentRosterInputHTML = (roster, className = 'span-2') => {
  ...
  return `<div class="${className} opponent-roster-input"><div class="opponent-roster-fields"><label><span class="field-label">背番号（1人1行）</span><textarea name="opponentRosterNumbersText" data-opponent-numbers inputmode="numeric" spellcheck="false" rows="5" placeholder="4&#10;7&#10;12">${esc(numbers)}</textarea></label><label><span class="field-label">名前（同じ行・任意）</span><textarea name="opponentRosterNamesText" rows="5" placeholder="選手A&#10;選手B&#10;（任意）">${esc(names)}</textarea></label></div><small class="field-help">左の背番号と右の名前を同じ行に入力してください。背番号は半角数字1〜3桁、名前は任意です。空行は無視されます。交代を使う場合はベンチ選手も入力してください。</small></div>`;
};
