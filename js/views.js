...
const opponentRosterNumbersText = value => String(value || '').split(/\r?\n/).map(line => {
...
export function gameFormView(s, d) {
  ...
  const opponentRosterText = opponentRosterNumbersText(d.opponentRosterText);
...
export function liveSettingsHTML(g) {
  ...
  const rosterText = (g.opponentRoster || []).map(player => player.number).join('\n');
  const radio = (name, value, label, checked) => `<label><input type="radio" name="${name}" value="${value}" ${checked ? 'checked' : ''}><span>${label}</span></label>`;
  return `<form id="live-settings-form"><p class="help">試合中でも記録モードとProの入力設定を変更できます。保存後すぐにLIVE画面へ戻ります。</p><label>記録モード<div class="segmented">${radio('mode', 'standard', '標準', mode === 'standard')}${radio('mode', 'pro', 'Pro', mode === 'pro')}</div></label><div class="live-settings-pro-options"><label class="spaced">ゲームクロック<div class="segmented">${radio('clockEnabled', 'off', '使わない', !clockEnabled)}${radio('clockEnabled', 'on', '使う', clockEnabled)}</div></label><label class="spaced">相手チームの記録<div class="segmented">${radio('opponentTracking', 'score', '総得点のみ', tracking === 'score')}${radio('opponentTracking', 'player', '個人も記録', tracking === 'player')}</div></label><label class="spaced">相手選手（個人記録用・1行に1人：背番号のみ）<textarea name="opponentRosterText" rows="4" maxlength="2400" placeholder="4\n7\n11">${esc(rosterText)}</textarea><small class="field-help">名前は記録せず、背番号だけで相手個人スタッツを記録します。</small></label></div><button class="button primary full spaced" type="submit">設定を保存</button></form>`;
}
