import { STATS, STAT_DEFS, SHOT_ZONES, aggregate, aggregateGames, activeEvents, eventLabel, formatGame, isPointShotEvent, lineup, localDate, normalizeShotZone, percent } from './domain.js';
...
const PRO_SHOT_ACTIONS = [['2PM', '2P', '○', 'made'], ['2PX', '2P', '×', 'miss'], ['3PM', '3P', '○', 'made'], ['3PX', '3P', '×', 'miss'], ['FTM', 'FT', '○', 'made'], ['FTX', 'FT', '×', 'miss']];
...
function proPlayerButton(player, selected, opponent = false) {
  return `<button class="pro-player ${selected ? 'selected' : ''}" data-action="${opponent ? 'pro-select-opponent' : 'pro-select-player'}" data-id="${esc(player.id)}" aria-pressed="${selected ? 'true' : 'false'}"><strong>#${esc(player.number)}</strong><span>${esc(player.name)}</span></button>`;
}
function proCourtHTML(g, events) {
  const markers = activeEvents(events).filter(event => isPointShotEvent(event) && Number.isFinite(event.shotX) && Number.isFinite(event.shotY)).map(event => {
    const x = event.shotX * 940, y = event.shotY * 500, made = event.eventType.endsWith('M');
    return `<g class="pro-shot-marker ${made ? 'made' : 'miss'}" transform="translate(${x.toFixed(1)} ${y.toFixed(1)})"><circle r="13"></circle><text text-anchor="middle" dy=".36em">${made ? '○' : '×'}</text><title>${esc(eventLabel(g, event))}</title></g>`;
  }).join('');
  return `<svg class="pro-court" viewBox="0 0 940 500" data-action="pro-shot-point" role="button" tabindex="0" aria-label="フルコート。シュート位置をタップ"><rect class="pro-court-surface" x="24" y="24" width="892" height="452" rx="2"></rect><g class="pro-court-markings"><path d="M24 24H916V476H24ZM470 24V476M24 250H916M70 168H170V332H70M870 168H770V332H870M170 168A120 120 0 0 0 170 332M770 168A120 120 0 0 1 770 332M70 250H48M870 250H892M48 237V263M892 237V263M24 110H125M24 390H125M916 110H815M916 390H815"></path><circle cx="70" cy="250" r="13"></circle><circle cx="870" cy="250" r="13"></circle></g><g class="pro-shot-markers">${markers}</g></svg>`;
}
export function proLiveView(s, g, events, clockSeconds = null) {
  const a = aggregate(g, events);
  const period = g.periods.find(p => p.id === g.currentPeriodId);
  const on = new Set(lineup(g, events));
  const selection = s.proSelection || {};
  const ownPlayers = g.roster.map(player => proPlayerButton(player, selection.playerId === player.id || s.proSub?.outPlayerId === player.id));
  const opponentSelection = s.proOpponentSelection || {};
  const opponentPlayers = (g.opponentRoster || []).map(player => proPlayerButton(player, opponentSelection.playerId === player.id, true));
  const selectedAction = selection.type ? STATS[selection.type]?.name || STATS[selection.type]?.label : '';
  const selectedPlayer = g.roster.find(player => player.id === selection.playerId);
  const selectedOpponentAction = opponentSelection.type ? STATS[opponentSelection.type]?.name || STATS[opponentSelection.type]?.label : '';
  const selectedOpponentPlayer = (g.opponentRoster || []).find(player => player.id === opponentSelection.playerId);
  const hint = s.proSub ? (s.proSub.outPlayerId ? '交代するベンチの選手をタップ' : '交代するコート上の選手をタップ') : selection.type && selection.playerId ? 'コート上をタップして位置を記録' : opponentSelection.type && opponentSelection.playerId ? 'コート上をタップして相手の位置を記録' : selection.type || opponentSelection.type ? '選手をタップ' : 'プレーを選択';
  const actionButton = ([type, label, symbol, tone]) => action('pro-action', `<span>${label}</span><b>${symbol}</b>`, `pro-action-button ${tone}`, `data-type="${type}" aria-label="${STATS[type].name}"`);
  ...
  const opponentAction = ([type, label, symbol, tone]) => action('pro-opponent-action', `<span>${label}</span><b>${symbol}</b>`, `pro-action-button ${tone}`, `data-type="${type}" aria-label="相手 ${STATS[type].name}"`);
  return `<main class="pro-live-screen"><header class="pro-header"><a href="#home" class="icon-button" aria-label="ホームに戻る">${icon('back')}</a><span class="live-title"><span class="live-dot"></span>PRO LIVE</span><span class="save-state">${icon('check')}保存済み</span>${action('game-menu', icon('more'), 'icon-button', 'aria-label="試合メニュー"')}</header><section class="pro-scoreboard" aria-label="スコア"><div><span>MY TEAM</span><strong>${esc(g.teamName)}</strong></div><b>${a.team.PTS}<i>–</i>${a.opponent}</b><div><span>OPPONENT</span><strong>${esc(g.opponentName)}</strong></div></section><div class="pro-meta"><span class="pro-period">${action('period-menu', `${esc(period.label)} ⌄`, 'period-button', 'aria-label="ピリオド操作"')}</span><span>${esc(formatGame(g))}</span>${g.clockEnabled ? `<button class="pro-clock ${g.clockRunning ? 'running' : ''}" data-action="pro-clock-toggle" aria-label="ゲームクロックを${g.clockRunning ? '停止' : '開始'}"><span id="pro-clock-value">${clockText(clockSeconds)}</span><small>${g.clockRunning ? 'STOP' : 'START'}</small></button><button class="pro-clock-reset" data-action="pro-clock-reset" aria-label="ゲームクロックをリセット">↺</button>` : '<span class="pro-clock-off">CLOCK OFF</span>'}</div><div class="pro-layout"><aside class="pro-side pro-home-side"><div class="pro-side-heading"><span>自チーム</span><strong>${a.team.PTS}</strong></div><p class="pro-roster-label">選手をタップ</p><div class="pro-roster">${ownPlayers.join('')}</div><button class="pro-sub-button" data-action="pro-sub">交代</button></aside><section class="pro-center"><div class="pro-court-status"><span>${esc(hint)}</span>${selectedPlayer ? `<b>#${esc(selectedPlayer.number)} ${esc(selectedPlayer.name)} · ${esc(selectedAction)}</b>` : selectedOpponentPlayer ? `<b>相手 #${esc(selectedOpponentPlayer.number)} ${esc(selectedOpponentPlayer.name)} · ${esc(selectedOpponentAction)}</b>` : ''}</div>${proCourtHTML(g, events)}<div class="pro-action-panel"><div class="pro-action-row">${PRO_SHOT_ACTIONS.map(actionButton).join('')}</div><div class="pro-action-row pro-other-row">${PRO_OTHER_ACTIONS.map(otherButton).join('')}</div></div></section><aside class="pro-side pro-away-side"><div class="pro-side-heading"><span>相手</span><strong>${a.opponent}</strong></div>${g.opponentTracking === 'player' && opponentPlayers.length ? `<p class="pro-roster-label">相手選手をタップ</p><div class="pro-roster">${opponentPlayers.join('')}</div><div class="pro-action-row pro-opponent-actions">${PRO_SHOT_ACTIONS.map(opponentAction).join('')}</div>` : `<p class="pro-roster-label">総得点のみ</p><div class="pro-opponent-score">${[1, 2, 3].map(points => action('opponent', `+${points}`, 'opponent-button', `data-points="${points}" aria-label="相手に${points}点追加"`)).join('')}</div>`}</aside></div><footer class="pro-footer">${action('undo', `${icon('undo')}<strong>UNDO</strong>`, 'undo-button', activeEvents(events).length ? '' : 'disabled')}<a href="#box/${g.id}" class="box-button">${icon('history')}<span>BOX SCORE</span></a></footer></main>`;
}
...
function fullCourtShotChartMapHTML(shots, playerId) {
  ...
  const markerHTML = shots.filter(shot => !playerId || shot?.playerId === playerId).map(shot => ({ raw: shot, normalized: chartShot(shot) })).filter(({ normalized }) => normalized?.x !== undefined && normalized?.y !== undefined).map(({ raw, normalized }) => `<g class="pro-shot-chart-marker ${normalized.made ? 'made' : 'miss'}" transform="translate(${(normalized.x * 940).toFixed(1)} ${(normalized.y * 500).toFixed(1)})"><circle r="13"></circle><text text-anchor="middle" dy=".36em">${normalized.made ? '○' : '×'}</text><title>${esc(raw?.zone || 'シュート')}</title></g>`).join('');
  ...
  return `<svg class="shot-court-map shot-chart-map pro-shot-chart-map" viewBox="0 0 940 500" role="img" aria-label="ショットチャート。各シュートの成功・失敗とエリア別の成功率"><rect class="pro-chart-court-surface" x="24" y="24" width="892" height="452" rx="2"></rect><g class="pro-chart-court-markings"><path d="M24 24H916V476H24ZM470 24V476M24 250H916M70 168H170V332H70M870 168H770V332H870M170 168A120 120 0 0 0 170 332M770 168A120 120 0 0 1 770 332M70 250H48M870 250H892M48 237V263M892 237V263M24 110H125M24 390H125M916 110H815M916 390H815"></path><circle cx="70" cy="250" r="13"></circle><circle cx="870" cy="250" r="13"></circle></g><g class="pro-shot-chart-markers">${markerHTML}</g></svg><div class="pro-shot-zone-summary">${summary}</div>`;
}
export function shotChartMapHTML(shots = [], playerId = null) {
  ...
  if (hasCoordinates) return fullCourtShotChartMapHTML(shots, playerId);
  const totals = shotChartTotals(shots, playerId);
  if (!Object.values(totals).some(stats => stats.attempts)) return '';
  const zones = SHOT_MAP_ZONES.map(zone => {
    const label = SHOT_ZONES.find(candidate => candidate.id === zone.id)?.label || zone.id;
