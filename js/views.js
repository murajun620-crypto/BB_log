...
export function proLiveView(s, g, events, clockSeconds = null) {
  ...
  const opponentPlayers = [...(g.opponentRoster || [])]
    ...
    .map(player => proPlayerButton(player, opponentSelection.playerId === player.id || s.proOpponentSub?.outPlayerId === player.id, true, opponentOnCourt.has(player.id), a.opponentPlayers[player.id]));
      ...
      : ownPointReady ? 'pro-await-own-court'
        : opponentPointReady ? 'pro-await-opponent-court'
          : selection.type ? 'pro-await-own-player'
            : opponentSelection.type ? 'pro-await-opponent-player'
              : 'pro-step-ready';
  ...
  const opponentAction = ([type, label, symbol, tone, name]) => action('pro-opponent-action', `<span>${label}</span><b>${symbol}</b>`, `pro-action-button ${tone} ${opponentSelection.type === type ? 'selected' : ''}`, `data-type="${type}" aria-label="相手 ${name}" aria-pressed="${opponentSelection.type === type ? 'true' : 'false'}"`);
  ...
  const opponentSide = g.opponentTracking === 'player'
    ? `<p class="pro-roster-label">OPPONENT</p><div class="pro-roster" aria-label="相手選手一覧">${opponentRoster}</div><button class="pro-sub-button opponent-sub-button" data-action="pro-opponent-sub" aria-label="相手チームの交代">相手交代</button><div class="pro-action-row pro-opponent-actions">${PRO_SHOT_ACTIONS.map(opponentAction).join('')}</div>`
