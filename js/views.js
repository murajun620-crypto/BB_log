...
export const BOX_STAT_KEYS = ['PTS', 'REB', 'AST', 'STL', 'BLK', 'TO', 'PF', 'FD'];
export const DETAIL_STAT_KEYS = ['OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TO', 'PF', 'FD'];
...
export function boxView(s, g, events) {
  ...
  const opponentRow = (p, st) => `<tr><th scope="row"><b>${esc(p.number)}</b>${p.name ? `<span>${esc(p.name)}</span>` : ''}</th>${BOX_STAT_KEYS.map(k => `<td ${k === 'PTS' ? 'class="pts-cell"' : ''}>${st[k]}</td>`).join('')}</tr>`;
