...
function proCourtHTML(g, events, interactive = false, selectionSide = null) {
  ...
  const shotDirection = selectionSide === 'opponent' ? (attackDirection === 'left' ? 'right' : 'left') : attackDirection;
  const backcourt = interactive && selectionSide ? shotDirection === 'right'
    ...
    : '';
  return `<svg class="pro-court" viewBox="0 0 940 500" ${interaction} aria-label="${interactive ? 'フルコート。フロントコートのシュート位置をタップ' : 'フルコート'}"><rect class="pro-court-surface" x="24" y="24" width="892" height="452" rx="2"></rect>${backcourt}<g class="pro-court-markings">${PRO_COURT_MARKINGS}</g><g class="pro-shot-markers">${markers}</g></svg>`;
}
...
const strategyBoardToolLabels = { player: '選手', ball: 'ボール', line: 'ライン', arrow: '矢印', erase: '消しゴム' };
...
export function strategyBoardItemsHTML(board = {}) {
  return (board.items || []).map(item => {
    const x = strategyBoardPoint(item.x); const y = Math.max(20, Math.min(580, Number(item.y) || 20));
    if (item.kind === 'marker') return item.marker === 'ball'
      ? `<g class="strategy-board-ball" data-strategy-item="${esc(item.id)}"><circle cx="${x}" cy="${y}" r="15"></circle><text x="${x}" y="${y + 6}" text-anchor="middle">●</text></g>`
      : `<g class="strategy-board-player" data-strategy-item="${esc(item.id)}"><circle cx="${x}" cy="${y}" r="22"></circle><text x="${x}" y="${y + 7}" text-anchor="middle">${esc(item.label)}</text></g>`;
    const startX = strategyBoardPoint(item.startX); const startY = Math.max(20, Math.min(580, Number(item.startY) || 20));
    const endX = strategyBoardPoint(item.endX); const endY = Math.max(20, Math.min(580, Number(item.endY) || 20));
...
export function strategyBoardDraftHTML(draft = null) {
  if (!draft) return '';
  return `<line class="strategy-board-draft ${draft.tool === 'arrow' ? 'arrow' : ''}" x1="${strategyBoardPoint(draft.startX)}" y1="${Math.max(20, Math.min(580, Number(draft.startY) || 20))}" x2="${strategyBoardPoint(draft.endX)}" y2="${Math.max(20, Math.min(580, Number(draft.endY) || 20))}"${draft.tool === 'arrow' ? ' marker-end="url(#strategy-board-arrow)"' : ''}></line>`;
}
export function strategyBoardHTML(board = {}) {
  const selected = strategyBoardToolLabels[board.tool] ? board.tool : 'player';
  ...
  return `<div class="strategy-board-toolbar"><div class="strategy-board-tools" role="group" aria-label="作戦ボードの道具">${tools}</div><div class="strategy-board-actions"><button type="button" class="strategy-board-action" data-action="strategy-undo">UNDO</button><button type="button" class="strategy-board-action danger" data-action="strategy-clear">全消去</button></div></div><p class="strategy-board-hint" id="strategy-board-hint">選手を選び、コートをタップして配置</p><svg class="strategy-board-canvas" data-strategy-board viewBox="0 0 1000 600" role="img" aria-label="作戦ボード。選手やボールを配置し、ラインと矢印を描けます"><defs><marker id="strategy-board-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"></path></marker></defs><rect class="strategy-board-surface" x="20" y="20" width="960" height="560" rx="4"></rect><g class="strategy-board-court"><path d="M20 20H980V580H20ZM500 20V580M20 88H194M20 512H194M194 88A255 255 0 0 1 194 512M980 88H806M980 512H806M806 88A255 255 0 0 0 806 512M20 230H250V370H20M980 230H750V370H980M250 250A60 60 0 0 1 250 350M750 250A60 60 0 0 0 750 350M96 270V330M904 270V330M250 230A42 42 0 0 1 250 370M750 230A42 42 0 0 0 750 370"></path><circle cx="500" cy="300" r="56"></circle><circle cx="96" cy="300" r="12"></circle><circle cx="904" cy="300" r="12"></circle></g><g data-strategy-board-items>${strategyBoardItemsHTML(board)}</g><g data-strategy-board-draft></g></svg><div class="strategy-board-footnote"><span>横向きで広く使えます。ボードはこの端末の試合画面だけに保持されます。</span></div>`;
}
