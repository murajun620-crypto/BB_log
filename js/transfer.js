import { aggregate, efficiency, percent, validateTeam, validateGame } from './domain.js';
...
export function parseBackup(text) {
  ...
  for (const s of data.settings) {
    ...
    keys.add(s.key);
    if (s.key === 'preferences' && (!s.value || !['system', 'light', 'dark'].includes(s.value.theme) || typeof s.value.continuous !== 'boolean' || (s.value.keepAwake !== undefined && typeof s.value.keepAwake !== 'boolean') || (s.value.advancedMode !== undefined && typeof s.value.advancedMode !== 'boolean'))) throw new Error('表示設定が不正です。');
    if (s.key === 'teamDraft') {
      if (!s.value || typeof s.value.name !== 'string' || !Array.isArray(s.value.players) || s.value.players.some(p => !p || typeof p.id !== 'string' || typeof p.number !== 'string' || typeof p.name !== 'string')) throw new Error('チーム下書きが不正です。');
    }
...
export function gameCSV(game, events) {
  ...
  const header = ['Date', 'Team', 'Opponent', 'Number', 'Player', 'PTS', 'EFF', 'FGM', 'FGA', 'FG%', '2PM', '2PA', '2P%', '3PM', '3PA', '3P%', 'FTM', 'FTA', 'FT%', 'OR', 'DR', 'REB', 'AST', 'STL', 'BLK', 'TO', 'F'];
  const line = (p, s) => [game.date, game.teamName, game.opponentName, p.number, p.name, s.PTS, efficiency(s), s.FGM, s.FGA, percent(s.FGM, s.FGA), s.P2M, s.P2A, percent(s.P2M, s.P2A), s.P3M, s.P3A, percent(s.P3M, s.P3A), s.FTM, s.FTA, percent(s.FTM, s.FTA), s.OREB, s.DREB, s.REB, s.AST, s.STL, s.BLK, s.TO, s.PF];
